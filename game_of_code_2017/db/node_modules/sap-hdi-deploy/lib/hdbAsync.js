...
function printMessagesToFile(logger, messages) {
    messages.forEach(function(message) {
        if (message.MESSAGE === 'END') {
            return;
        }
...
function printMessages(logger, messages) {
    ...
    printMessagesToFile(logger, messages);
}
...
module.exports = function(host, port, user, password, certificate) {
    var credentials = {
            host: host,
            port: port,
            user: user,
    ...
    return {
        ...
        grantSystemPrivileges: function(privileges, grantee) {
            return execute('GRANT ' + privileges.map(identifier).join(',') + ' TO ' + identifier(grantee));
        },
