...
var sqlInjectionUtils = require('./sqlInjectionUtils');

var escapeSingleQuotes = sqlInjectionUtils.escapeSingleQuotes;
var escapeDoubleQuotes = sqlInjectionUtils.escapeDoubleQuotes;
...
function setSchema(client, schema, cb) {
  ...
  var sql = 'SET SCHEMA "' + escapeDoubleQuotes(schema) + '"';
  logger.debug('setting schema: ' + sql);
  client.exec(sql, cb);
}
...
function setSessionVariable(client, key, value, cb) {
  ...
  var sql = util.format("SET '%s' = '%s'", escapeSingleQuotes(key), escapeSingleQuotes(value));
  client.exec(sql, cb);
}

function unsetSessionVariable(client, key, cb) {
  var sql = util.format("UNSET '%s'", escapeSingleQuotes(key));
  client.exec(sql, cb);
}
