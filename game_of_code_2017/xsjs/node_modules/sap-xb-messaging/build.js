...
var modPath = platform+ '-'+ arch+ '-'+ os.endianness()+ '-v8-'+ v8;
if (!force) {
	try {
		fs.statSync(path.join(__dirname, 'bin', modPath, 'xbnode.node'));
	...
	.on('exit', function(err) {
		if (err) {
			console.error(
				...
				'would like to compile xb-messaging on this machine please make sure you have setup your\n'+
				'build environment--\n'+
...
function afterBuild() {
	var targetPath = path.join(__dirname, 'build', debug ? 'Debug' : 'Release', 'xbnode.node');
	var installPath = path.join(__dirname, 'bin', modPath, 'xbnode.node');
