...
module.exports = {
  ...
  refreshToken: function(app, session, requestUrl) {
    ...
    memoryStore.getSessionTimeout(session.id, function(err, sessionTimeout) {
      ...
      memoryStore.update(session.id, function(newSession) {
        ...
        module.exports.executeAfter(function() {
          doRefresh(app, newSession, requestUrl);
        }, msBeforeRefresh);
      }, false);
    });
  }
...
function doRefresh(app, session, requestUrl) {
  var routerConfig = app.get('routerConfig');
  var uaaOptions = routerConfig.uaaOptions;
  var tokenUrl = uaaUtils.getUaaUrl(routerConfig, requestUrl) + '/oauth/token';
  var requestOptions = {
    url: tokenUrl,
    form: {
