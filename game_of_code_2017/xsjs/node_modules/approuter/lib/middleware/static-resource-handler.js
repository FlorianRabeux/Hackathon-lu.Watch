...
module.exports = function staticResourceHandler(req, res, next) {
  ...
  fs.realpath(filePath, function (err, resolvedPath) {
    ...
    if (isMatching) {
      serveReplacedFile(resolvedPath, req, res, next, replacements.varsMap);
    } else {
...
function serveReplacedFile(filePath, req, res, next, vars) {
  fs.readFile(filePath, 'utf8', function (err, data) {
    ...
    var responseData;
    try {
      responseData = mustache.render(data, vars);
    } catch (error) {
...
function sendError(req, err, next, status, filePath) {
  filePath && req.loggingContext.getTracer().info('cannot read file: ' + filePath);
  err.status = status;
  next(err);
}
