...
function load(client, schema, name, cb) {
  ...
  async.waterfall([
    ...
    function(schema, callback) {
      var sql = 'SELECT PARAMETER_NAME, DATA_TYPE_NAME, PARAMETER_TYPE, TABLE_TYPE_SCHEMA, TABLE_TYPE_NAME' +
...
StoredProcedure.prototype._createTempTable = function(tempTable, metadata, input, cb) {
  var self = this;
  var sql = 'create local temporary table ' + tempTable + ' like "' +
    metadata.TABLE_TYPE_SCHEMA + '"."' + metadata.TABLE_TYPE_NAME + '"';
  debug(sql);
  this._client.execute(sql, function(err) {
    if (err) {
      return cb(err);
    }
